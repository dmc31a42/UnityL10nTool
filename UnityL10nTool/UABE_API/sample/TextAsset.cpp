	...
	if (importElements.size() > 0)
	{
		...
		if (doImport)
		{
			...
			for (size_t i = 0; i < batchDesc.importFilePaths.size(); i++)
			{
				...
				if (!batchDesc.importFilePaths[i].empty())
				{
					TCHAR progPrepend[32];
					_stprintf_s(progPrepend, _T("%u/%u"), actualImportIndex + 1, actualImportCount);
					if (pProgress)
					{
						if (pProgress->IsCancelled())
							break;
						pProgress->SetStepStatus(actualImportIndex);
						pProgress->SetDescription(std::basic_string<TCHAR>(progPrepend) + _T(" Importing"));
					}
					...
					if (pSourceReader)
					{
						...
						if (pFileBuffer)
						{
							...
							if (pInterface->MakeTemplateField(pAsset->GetAssetsFile(), pAsset->GetClassID(), &templateBase))
							{
								...
								if (pBase)
								{
									...
									else
									{
										if (pProgress)
										{
											pProgress->AddLogLine(std::basic_string<TCHAR>(progPrepend) + _T(" ERROR: Unknown TextAsset format!"));
										}
										else
								...
								else
								{
									if (pProgress)
									{
										pProgress->AddLogLine(std::basic_string<TCHAR>(progPrepend) + _T(" ERROR: Unable to load the TextAsset!"));
									}
									else
							...
							else
							{
								if (pProgress)
								{
									pProgress->AddLogLine(std::basic_string<TCHAR>(progPrepend) + _T(" ERROR: TextAsset format not found!"));
								}
								else
...
void ExportToTXT(HWND hParentWnd, IPluginInterface *pInterface, std::vector<IElementDescriptor*> &elements, const std::vector<const char*> &commandLine)
{
	...
	for (size_t i = 0; i < elements.size(); i++)
	{
		...
		TCHAR progPrepend[64];
		_stprintf_s(progPrepend, _T("%u/%u"), (unsigned int)(exportIndex + 1), (unsigned int)exportCount);
		...
		if (pProgress)
		{
			...
			pProgress->SetDescription(std::basic_string<TCHAR>(progPrepend) + _T(" Reading asset..."));
				...
				if (!nameField->IsDummy() && !scriptField->IsDummy() && !dataArrayField->IsDummy())
				{
					...
					if (dlgTargetFileName != NULL)
					{
						...
						if (pFileWriter != NULL)
						{
							if (pProgress)
							{
								pProgress->SetDescription(std::basic_string<TCHAR>(progPrepend)  + _T(" Exporting TextAsset to file..."));
								//pProgress->AddLogLine(description);
						...
						else
						{
							if (pProgress)
							{
								pProgress->AddLogLine(std::basic_string<TCHAR>(progPrepend) + _T(" ERROR: Unable to open the output file!"));
							}
							else
